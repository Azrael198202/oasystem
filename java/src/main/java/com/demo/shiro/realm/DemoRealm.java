...
public class DemoRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		if (principals != null) {
			...
			if (PrimaryPrincipal instanceof UserMaster) {
				...
				companyCode = userMaster.getCompanyCode();

			} else {

				// 此处的principals为token
				userid = JwtUtil.getClaim(principals.toString(), Const.TOKEN_CLAIM_USERID);
				companyCode = JwtUtil.getClaim(principals.toString(), Const.COMPANY_CODE);
			}
		}

		SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();

		UserAccessInfo userAccessInfo = iUserService.getUserAccessInfo(companyCode,userid);
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
		...
		String refreshTokenKey = RedisConst.PREFIX_SHIRO_REFRESH_TOKEN + userKey ;
